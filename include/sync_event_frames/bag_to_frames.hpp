...
#ifndef SYNC_EVENT_FRAMES__BAG_TO_FRAMES_HPP_
#define SYNC_EVENT_FRAMES__BAG_TO_FRAMES_HPP_
...
struct BagToFrames
{
    ...
    double fillRatio, bool hasSyncCable, double fps, bool writePNG)
  {
    ...
    for (size_t i = 0; i < inTopics.size(); i++) {
      recons.insert(
        {inTopics[i],
         ApproxRecon(&writer, outTopics[i], cutoffPeriod, fillRatio)});
    }
...
#endif  // SYNC_EVENT_FRAMES__BAG_TO_FRAMES_HPP_
